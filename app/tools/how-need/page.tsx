...
function HowNeedPage() {
    const [goalResource, setGoalResource] = useState<string>("");
    const [resourceKind, setResourceKind] = useState<string>("gacha-count");
    const [oneTimeResource, setOneTimeResource] = useState<string>("");
    ...
    const needResource = useMemo(() => {
        if (!goalResource || !oneTimeResource) return 0;
        if (resourceKind === "gacha-count") {
            return parseInt(goalResource) * parseInt(oneTimeResource);
        }
        return parseInt(goalResource);
    ...
    const oneDayNorma = useMemo(() => {
        if (!needResource || !currentResource) return 0;
        const remainingResource = needResource - parseInt(currentResource);
    ...
    const goalGachaCount = useMemo(() => {
        ...
        return Math.floor(needResource / parseInt(oneTimeResource));
    }, [needResource, oneTimeResource]);

    const currentGachaCount = useMemo(() => {
        if (!currentResource || !oneTimeResource) return 0;
        return Math.floor(
            parseInt(currentResource) / parseInt(oneTimeResource)
        );
    ...
    useEffect(() => {
        if (typeof window !== "undefined") {
            ...
            setGoalResource(params.get("goalResource") ?? "0");
            setResourceKind(params.get("resourceKind") ?? "gacha-count");
            setOneTimeResource(params.get("oneTimeResource") ?? "0");
            setGoalDate(
                params.get("goalDate")
                    ? new Date(decodeURIComponent(params.get("goalDate")!))
                    : new Date()
            );
            setCurrentResource(params.get("currentResource") ?? "0");
        }
    }, []);
                ...
                <CardContent className="space-y-6">
                    ...
                    <div>
                        ...
                        <Input
                            ...
                            placeholder="100"
                            value={goalResource}
                            onChange={(e) => setGoalResource(e.target.value)}
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <Input
                            ...
                            onChange={(e) => setOneTimeResource(e.target.value)}
                        />
                    </div>
                    <div>
                        <Label>現在のコイン数</Label>
